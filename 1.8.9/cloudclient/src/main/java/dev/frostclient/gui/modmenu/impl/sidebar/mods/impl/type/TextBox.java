...
public class TextBox extends Settings {
    ...
    public void renderSetting(int mouseX, int mouseY) {
        ...
        if (setting.getText().equals("")) {
            Frost.INSTANCE.fontHelper.size20.drawString(setting.getPlaceholderText(), x + offset + 5, y + offset + 4, 0x50ffffff);
        } else {
            Frost.INSTANCE.fontHelper.size20.drawString(setting.getText(), x + offset + 5, y + offset + 4, -1);
            Helper2D.drawRectangle(x + offset + 5 + Frost.INSTANCE.fontHelper.size20.getStringWidth(setting.getText().substring(0, setting.getCursorPosition())), y + offset + 3, 1, 10, 0x80ffffff);
            if (allSelected) {
                Helper2D.drawRectangle(x + offset + 2, y + offset + 1, Frost.INSTANCE.fontHelper.size20.getStringWidth(setting.getText()) + 4, 14, 0x503030ff);
            }
        }
        GLHelper.endScissor();
    }
