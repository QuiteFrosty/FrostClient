...
import dev.frostclient.feature.mod.impl.ToggleSneakMod;
        ...
        if (Frost.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Frost.INSTANCE.fontHelper.size20.drawString(
                        ...
                        getX() + getW() / 2f - Frost.INSTANCE.fontHelper.size20.getStringWidth("Sneak: " + (isSneaking() ? "Toggled" : "Vanilla")) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Frost.INSTANCE.mc.fontRendererObj.drawString(
                        "Sneak: " + (isSneaking() ? "Toggled" : "Vanilla"),
                        getX() + getW() / 2 - Frost.INSTANCE.mc.fontRendererObj.getStringWidth("Sneak: " + (isSneaking() ? "Toggled" : "Vanilla")) / 2,
                        getY() + 6,
                        getColor()
                );
            }
