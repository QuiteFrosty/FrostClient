...
public class CpsHud extends HudMod {
        ...
        if (Frost.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Frost.INSTANCE.fontHelper.size20.drawString(
                        ...
                        getX() + getW() / 2f - Frost.INSTANCE.fontHelper.size20.getStringWidth(isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Frost.INSTANCE.mc.fontRendererObj.drawString(
                        isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS(),
                        getX() + getW() / 2 - Frost.INSTANCE.mc.fontRendererObj.getStringWidth(isRightClick() ? "CPS: " + getLeftCPS() + " | " + getRightCPS() : "CPS: " + getLeftCPS()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private boolean isRightClick() {
        return Frost.INSTANCE.settingManager.getSettingByModAndName(getName(), "Right click").isCheckToggled();
    }

    private int getLeftCPS() {
        return Frost.INSTANCE.cpsHelper.getCPS(0);
    }

    private int getRightCPS() {
        return Frost.INSTANCE.cpsHelper.getCPS(1);
    }
}
