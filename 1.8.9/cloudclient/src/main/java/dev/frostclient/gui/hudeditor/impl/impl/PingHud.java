...
public class PingHud extends HudMod {
        ...
        if (Frost.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Frost.INSTANCE.fontHelper.size20.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2f - Frost.INSTANCE.fontHelper.size20.getStringWidth("Ping: " + getPing()) / 2f,
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Frost.INSTANCE.mc.fontRendererObj.drawString(
                        "Ping: " + getPing(),
                        getX() + getW() / 2 - Frost.INSTANCE.mc.fontRendererObj.getStringWidth("Ping: " + getPing()) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private long getPing() {
        return Frost.INSTANCE.mc.getCurrentServerData() == null ? 0 : Frost.INSTANCE.mc.getCurrentServerData().pingToServer;
    }
