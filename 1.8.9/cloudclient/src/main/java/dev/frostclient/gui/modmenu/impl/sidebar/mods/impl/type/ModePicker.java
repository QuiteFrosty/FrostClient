...
public class ModePicker extends Settings {
    ...
    public void renderSetting(int mouseX, int mouseY) {
        ...
        Frost.INSTANCE.fontHelper.size20.drawString(
                ...
                button.getPanel().getX() + button.getPanel().getW() - 20 - Frost.INSTANCE.fontHelper.size20.getStringWidth(setting.getCurrentMode()),
        ...
        if (isOpen()) {
            ...
            for (int i = 0; i < setting.getOptions().length; i++) {
                ...
                if (longestString < Frost.INSTANCE.fontHelper.size20.getStringWidth(setting.getOptions()[i])) {
                    longestString = Frost.INSTANCE.fontHelper.size20.getStringWidth(setting.getOptions()[i]);
                }
                offset++;
            }
            if (!animateSelect.hasFinished()) {
