...
package dev.frostclient.gui.titlescreen.buttons;
...
public class TextButton {
    ...
    public void renderButton(int x, int y, int mouseX, int mouseY) {
        ...
        Helper2D.drawRoundedRectangle(x, y, w, h, 2,
                ...
                Frost.INSTANCE.optionManager.getOptionByName("Rounded Corners").isCheckToggled() ? 0 : -1
        );

        Frost.INSTANCE.fontHelper.size20.drawString(
                text,
                x + w / 2f - Frost.INSTANCE.fontHelper.size20.getStringWidth(text) / 2f,
                y + h / 2f - 4,
                -1
        );
    }
