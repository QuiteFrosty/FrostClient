...
package dev.frostclient.gui.hudeditor;

import dev.frostclient.Frost;
import dev.frostclient.gui.Style;
import dev.frostclient.gui.hudeditor.impl.HudMod;
import dev.frostclient.gui.hudeditor.impl.impl.*;
import dev.frostclient.gui.hudeditor.impl.impl.keystrokes.KeystrokesHud;
import dev.frostclient.gui.modmenu.ModMenu;
...
public class HudEditor extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        Frost.INSTANCE.fontHelper.size40.drawString(
                "Frost Client",
                width / 2f - Frost.INSTANCE.fontHelper.size40.getStringWidth("Frost Client") / 2f,
        ...
        Helper2D.drawPicture(
                ...
                50, 50, Style.getColor(70).getRGB(), "frostlogo.png"
        );
        GLHelper.endScissor();
        ...
        Frost.INSTANCE.fontHelper.size20.drawString(
                "Open Mods",
                width / 2f - Frost.INSTANCE.fontHelper.size20.getStringWidth("Open Mods") / 2f,
                height / 2f,
                color
        );
        ...
        for (HudMod hudMod : hudModList) {
            ...
            for (HudMod sHudMod : hudModList) {
                if (
                        Frost.INSTANCE.modManager.getMod(sHudMod.getName()).isToggled() &&
                                hudMod.isDragging() &&
    ...
    public void onKey(InputEvent.KeyInputEvent e) {
        if (Keyboard.isKeyDown(Frost.INSTANCE.optionManager.getOptionByName("ModMenu Keybinding").getKey())) {
