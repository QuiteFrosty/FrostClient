...
public class ScoreboardHud extends HudMod {
    ...
    public void onRender2D(RenderGameOverlayEvent.Pre.Text e) {
        ...
        if (Frost.INSTANCE.modManager.getMod(getName()).isToggled() && !(Frost.INSTANCE.mc.currentScreen instanceof HudEditor)) {
            ScoreObjective scoreobjective = Frost.INSTANCE.mc.theWorld.getScoreboard().getObjectiveInDisplaySlot(1);
    ...
    private void drawScoreboard(ScoreObjective objective, boolean background, boolean numbers) {
        ...
        int displayText = Frost.INSTANCE.mc.fontRendererObj.getStringWidth(objective.getDisplayName()) + 2;
        ...
        for (Score score : collection) {
            ...
            displayText = Math.max(displayText, Frost.INSTANCE.mc.fontRendererObj.getStringWidth(text));
        }

        int y = getY();
        int x = getX();

        int textHeight = Frost.INSTANCE.mc.fontRendererObj.FONT_HEIGHT;
        ...
        for (Score score1 : collection) {
            ...
            if (index == 0) {
                ...
                Frost.INSTANCE.mc.fontRendererObj.drawString(topText, x + 2 + displayText / 2 - Frost.INSTANCE.mc.fontRendererObj.getStringWidth(topText) / 2, calculatedY + 1, -1);
            }

            if (background) {
                Helper2D.drawRectangle(x, calculatedY + textHeight + 1, displayText + 4, textHeight, 1342177280);
            }
            Frost.INSTANCE.mc.fontRendererObj.drawString(mainText, x + 2, calculatedY + textHeight + 1, -1);
            if (!numbers) {
                Frost.INSTANCE.mc.fontRendererObj.drawString(redNumbers, x + 4 + displayText - Frost.INSTANCE.mc.fontRendererObj.getStringWidth(redNumbers), calculatedY + textHeight + 1, -1);
            }
    ...
    private void drawScoreboardPlaceHolder(boolean background, boolean numbers) {
        ...
        int displayText = Frost.INSTANCE.mc.fontRendererObj.getStringWidth(objective) + 2;
        ...
        for (int i = 0; i < collectionSize; i++) {
            ...
            if (index == 0) {
                ...
                Frost.INSTANCE.mc.fontRendererObj.drawString(objective, x + 2 + displayText / 2 - Frost.INSTANCE.mc.fontRendererObj.getStringWidth(objective) / 2, calculatedY + 1, -1);
            }
    ...
    private boolean isRemoveNumbers() {
        return Frost.INSTANCE.settingManager.getSettingByModAndName(getName(), "Remove Red Numbers").isCheckToggled();
    }
}
