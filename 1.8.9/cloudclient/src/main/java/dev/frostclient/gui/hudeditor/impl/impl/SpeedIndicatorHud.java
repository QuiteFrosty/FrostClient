...
public class SpeedIndicatorHud extends HudMod {
        ...
        if (Frost.INSTANCE.modManager.getMod(getName()).isToggled()) {
            if (isModern()) {
                ...
                Frost.INSTANCE.fontHelper.size20.drawString(
                        ...
                        getX() + getW() / 2f - (Frost.INSTANCE.fontHelper.size20.getStringWidth((getMeterPerSecond() + " m/s")) / 2f),
                        getY() + 6,
                        getColor()
                );
            } else {
                ...
                Frost.INSTANCE.mc.fontRendererObj.drawString(
                        getMeterPerSecond() + " m/s",
                        getX() + getW() / 2 - (Frost.INSTANCE.mc.fontRendererObj.getStringWidth(getMeterPerSecond() + " m/s")) / 2,
                        getY() + 6,
                        getColor()
                );
            }
    ...
    private double getMeterPerSecond() {
        double x = Frost.INSTANCE.mc.thePlayer.posX - Frost.INSTANCE.mc.thePlayer.prevPosX;
        double z = Frost.INSTANCE.mc.thePlayer.posZ - Frost.INSTANCE.mc.thePlayer.prevPosZ;
