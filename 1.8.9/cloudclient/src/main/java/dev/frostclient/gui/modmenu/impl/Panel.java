...
package dev.frostclient.gui.modmenu.impl;
...
import dev.frostclient.gui.modmenu.impl.sidebar.options.type.*;
import dev.frostclient.gui.modmenu.impl.sidebar.TextBox;
...
public class Panel {
    ...
    public void renderPanel(int mouseX, int mouseY) {
        ...
        Helper2D.drawPicture(x + 2, y - 1, 35, 35, color, "frostlogo.png");
        Frost.INSTANCE.fontHelper.size40.drawString("Frost Client", x + 37, y + 6, color);
        ...
        Frost.INSTANCE.messageHelper.renderMessage();

        if (selected == 0) {
            int offset = 0;
            for (Type type : Type.values()) {
                String text = type.name();
                int length = Frost.INSTANCE.fontHelper.size20.getStringWidth(text);
                ...
                Frost.INSTANCE.fontHelper.size20.drawString(text, x + offset + 26, y + h + 11, -1);
                offset += length + 30;
            }
        ...
        for (String button : sideButtons) {
            Frost.INSTANCE.fontHelper.size15.drawString(button, x - 30 - Frost.INSTANCE.fontHelper.size15.getStringWidth(button) / 2f, y + 30 + index * 40, color);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseButton == 0) {
            ...
            if (MathHelper.withinBox(x + w - 25, y + 5, 20, 20, mouseX, mouseY)) {
                Frost.INSTANCE.mc.displayGuiScreen(Frost.INSTANCE.hudEditor);
            }
        }
