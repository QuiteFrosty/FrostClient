...
public abstract class RenderMixin {
    ...
    protected void renderLivingLabel(Entity entityIn, String str, double x, double y, double z, int maxDistance) {
        ...
        boolean nameTagToggled = Frost.INSTANCE.modManager.getMod("NameTag").isToggled();
        int color = Frost.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Font Color").getColor().getRGB();
        float alpha = nameTagToggled ? Frost.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Opacity").getCurrentNumber() / 255f : 0.25f;
        float scale = nameTagToggled ? Frost.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Size").getCurrentNumber() : 1;
        float yPos = nameTagToggled ? Frost.INSTANCE.settingManager.getSettingByModAndName("NameTag", "Y Position").getCurrentNumber() - 2.5f : 0;
